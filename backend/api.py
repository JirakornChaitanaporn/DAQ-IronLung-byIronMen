...
def get_kidbright_outdoor_data():
    with get_cursor() as cursor:
        cursor.execute("SELECT id, ts, temp_dht, pm1, pm25, pm10 FROM project_kidbright_outdoor ORDER BY ts ASC")
...
def get_kidbright_outdoor_last_hour():
    with get_cursor() as cursor:
        cursor.execute("SELECT id, ts, temp_dht, pm1, pm25, pm10 FROM project_kidbright_outdoor WHERE ts >= NOW() - INTERVAL 1 HOUR ORDER BY ts ASC")
        data = cursor.fetchall()
    return data
...
def get_kidbright_outdoor_last_24hour():
    with get_cursor() as cursor:
        cursor.execute("SELECT id, ts, temp_dht, pm1, pm25, pm10 FROM project_kidbright_outdoor WHERE ts >= NOW() - INTERVAL 24 HOUR ORDER BY ts ASC")
        data = cursor.fetchall()
    return data
...
def get_kidbright_indoor_data():
    with get_cursor() as cursor:
        cursor.execute("SELECT id, ts, temp_dht, pm1, pm25, pm10 FROM project_kidbright_indoor ORDER BY ts ASC")
        data = cursor.fetchall()
    return interpolate_sensor_data(data)
...
def get_kidbright_indoor_last_hour():
    with get_cursor() as cursor:
        cursor.execute("SELECT id, ts, temp_dht, pm1, pm25, pm10 FROM project_kidbright_indoor WHERE ts >= NOW() - INTERVAL 1 HOUR ORDER BY ts ASC")
        data = cursor.fetchall()
    return data
...
def get_kidbright_indoor_last_24hour():
    with get_cursor() as cursor:
        cursor.execute("SELECT id, ts, temp_dht, pm1, pm25, pm10 FROM project_kidbright_indoor WHERE ts >= NOW() - INTERVAL 24 HOUR ORDER BY ts ASC")
        data = cursor.fetchall()
    return data
...
def get_aqi_api():
    with get_cursor() as cursor:
        cursor.execute("SELECT lat, lon, aqi FROM project_aqi_api")
        data = cursor.fetchall()
    return data
...
def get_aqi_api_last_24hour():
    with get_cursor() as cursor:
        cursor.execute("SELECT lat, lon, aqi FROM project_aqi_api WHERE ts >= NOW() - INTERVAL 24 HOUR")
        data = cursor.fetchall()
    return data
...
def get_weather_api():
    with get_cursor() as cursor:
        cursor.execute("SELECT ts, lat, lon, humid, rainfall, temp, windspeed FROM project_weather_api ORDER BY ts ASC")
        data = cursor.fetchall()
    return data
...
def get_weather_api_last_24hour():
    with get_cursor() as cursor:
        cursor.execute("SELECT ts, lat, lon, humid, rainfall, temp, windspeed FROM project_weather_api WHERE ts >= NOW() - INTERVAL 24 HOUR")
        data = cursor.fetchall()
    return data

@app.get("/predicted_indoor_lr")
def get_predicted_indoor_lr():
    with get_cursor() as cursor:
        cursor.execute("SELECT temp_dht, pm1, pm10, pm25 FROM project_kidbright_outdoor ORDER BY id DESC LIMIT 1")
        outdoor = cursor.fetchone()
        cursor.execute("SELECT humid, windspeed FROM project_weather_api ORDER BY id DESC LIMIT 1")
        weather = cursor.fetchone()
        cursor.execute("SELECT aqi FROM project_aqi_api ORDER BY id DESC LIMIT 1")
        aqi = cursor.fetchone()
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    return {
        "ts": ts,
        "pm1":  round(lr_predictor.predict_pm1( outdoor["pm1"],  weather["windspeed"], aqi["aqi"], outdoor["temp_dht"], weather["humid"])),
        "pm10": round(lr_predictor.predict_pm10(outdoor["pm10"], weather["windspeed"], aqi["aqi"], outdoor["temp_dht"], weather["humid"])),
        "pm25": round(lr_predictor.predict_pm25(outdoor["pm25"], weather["windspeed"], aqi["aqi"], outdoor["temp_dht"], weather["humid"])),
    }
...
def suggestion():
    ...
    return {"ts": ts, "Suggestion": Suggest.suggest([indoor_pm1, indoor_pm10, indoor_pm125], [outdoor["pm1"], outdoor["pm10"], outdoor["pm25"]])}