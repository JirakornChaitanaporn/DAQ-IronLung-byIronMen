...
from models.pm1_predictor import predict
...
# ===========================================================================
print("\n-- Monotonicity: higher outdoor PM1 -> higher indoor PM1 ---------")
# ===========================================================================
...
run("higher outdoor PM1 gives higher prediction",   test_higher_outdoor_pm1_increases_prediction)
run("predictions increase across PM1 steps",        test_monotonic_pm1_outdoor_steps)
...
run("high wind does not drastically lower prediction",  test_high_wind_not_lower_than_calm_by_large_margin)
run("windspeed=0 still gives a valid prediction",       test_windspeed_zero_still_returns_value)
...
run("higher AQI does not sharply lower prediction",  test_high_aqi_not_lower_than_low_aqi)
run("all AQI values return valid outputs",           test_aqi_range_gives_valid_output)
...
run("all outdoor temp values return valid outputs",  test_extreme_temp_still_returns_valid)
...
run("all humidity values return valid outputs",  test_humidity_range_gives_valid_output)
...
run("same input always gives same output",  test_deterministic_same_input)
run("different inputs give different outputs", test_different_inputs_give_different_outputs)
